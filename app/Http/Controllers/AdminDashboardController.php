...
class AdminDashboardController extends Controller
{
    ...
    public function dashboard(){
        ...
        $witTotalCharge = Withdraw::where('status', 1)->sum('fee');
    ...
    public function UserDetailsUpdate(Request $request)    {
        ...
        $msg = "";
         if($request->is_approved==1){
             $msg = "Your account has been disabled! Please contact the bank support for more information.";
         }else{
              $msg = " Your account has been approved successfully! These are your login details: Username:$request->username  Password: $data->password";
         }
         InfobipSms::send('0726582228', 'Hello Simon');
            send_email($request->email, $request->first_name, 'IMPORTANT!', $msg);
    ...
    public function addSub(Request $request, $id)
    {
        ...
        if ($request->type == 'on'){
            ...
            $tlog['amount'] = $user->amount;
            $tlog['balance'] = $user->balance;
            $tlog['type'] = 2;
            $tlog['details'] = $request->description;
            $tlog['trxid'] = Str::random(16);
            $tlog->save();

            $msg =  $request->description;
            send_email($user->email, $user->username, 'Your Account Has been Credited', $msg);
            $sms =  $request->description;
            send_sms($user->mobile, $sms);

            return redirect()->back()->withSuccess('Credited Successfully With '.' '. $request->amount);
            ...
            $msg =  $request->description;
            send_email($user->email, $user->username, 'Your Account Has been Debited', $msg);
            $sms =   $request->description;
            send_sms($user->mobile, $sms);

            return redirect()->back()->withSuccess('Debited Successfully with '.' '. $request->amount);
    ...
    public function sendMailToUser(Request $request, $id)
    {
        ...
        send_email($user->email, $user->username, $request->subject, $request->message);
        ...
        public function deleteUser(Request $request)
    {
        ...
        if($user->delete()){
              return redirect('/admin/allUser')->withSuccess($user->username .' '. ' Deleted Successfully! ' );
        }else{
              return redirect()->back()->withErrors('Failed to Delete '.' '. $user->username);
        }
